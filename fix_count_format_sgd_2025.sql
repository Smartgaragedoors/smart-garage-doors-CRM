-- Fix Count Format to Match Existing SGD-2025XXXX Format
-- Run this in your Supabase SQL editor

-- 1. Check current Count values to see the format difference
...
-- 3. Extract numeric part from SGD-2025 Count values
SELECT 
    "Count",
    SUBSTRING("Count" FROM 9) as numeric_part,
...
-- 4. Find the highest numeric value from SGD-2025 prefixed Counts
SELECT MAX(CAST(SUBSTRING("Count" FROM 9) AS INTEGER)) as highest_sgd_2025_numeric
FROM all_jobs 
WHERE "Count" LIKE 'SGD-2025%';
...
BEGIN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING("Count" FROM 9) AS INTEGER)), 0) INTO max_sgd_2025_numeric
    FROM all_jobs 
    WHERE "Count" LIKE 'SGD-2025%';
...
SELECT 
    ...
    (SELECT MAX(CAST(SUBSTRING("Count" FROM 9) AS INTEGER)) FROM all_jobs WHERE "Count" LIKE 'SGD-2025%') as highest_sgd_2025_numeric,
